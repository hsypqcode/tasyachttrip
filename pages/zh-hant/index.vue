<template>
  <div class="home-page">
    <section class="hero">
      <div class="hero-bg">
        <img src="https://booking-now.oss-accelerate.aliyuncs.com/uploads/1756394954077-65781745732245_.pic.jpg" alt="霍巴特港风景，从我们的钓鱼船上欣赏" />
      </div>
      <div class="hero-content">
        <div class="container">
          ...
          <div class="hero-actions">
            ...
            <NuxtLink :to="localePath('/zh/products')" class="btn btn-outline btn-lg">{{ $t('home.learnMore') }}</NuxtLink>
          </div>
        </div>
      </div>
    </section>
    ...
    <UiSection :title="$t('home.featuredTours')">
      <div class="products-grid">
        <UiCard hoverable>
          <template #image>
            <img src="https://diandian-1300713889.cos.ap-shanghai.myqcloud.com/IMG_7232.JPG" alt="客人在3小时霍巴特观光海钓巡游中钓鱼" loading="lazy" />
          </template>
          <template #title>{{ $t('products.sightseeingTitle') }}</template>
          <p>{{ $t('products.sightseeingDesc') }}</p>
          <template #footer>
            <NuxtLink :to="localePath('/zh/products/sightseeing-fishing-cruise')" class="btn-link">{{ $t('home.learnMore') }} →</NuxtLink>
          </template>
        </UiCard>

        <UiCard hoverable>
          <template #image>
            <img src="https://diandian-1300713889.cos.ap-shanghai.myqcloud.com/WechatIMG2935.jpeg" alt="塔斯马尼亚私人包船" loading="lazy" />
          </template>
          <template #title>{{ $t('products.privateTitle') }}</template>
          <p>{{ $t('products.privateDesc') }}</p>
          <template #footer>
            <NuxtLink :to="localePath('/zh/products/private-charters')" class="btn-link">{{ $t('home.learnMore') }} →</NuxtLink>
          </template>
        </UiCard>

        <UiCard hoverable>
          <template #image>
            <img src="https://diandian-1300713889.cos.ap-shanghai.myqcloud.com/IMG_5865.JPG" alt="半日钓鱼潜水烧烤行程中的海鲜烧烤午餐" loading="lazy" />
          </template>
          <template #title>{{ $t('products.halfDayTitle') }}</template>
          <p>{{ $t('products.halfDayDesc') }}</p>
          <template #footer>
            <NuxtLink :to="localePath('/zh/products/half-day-hook-dive-grill')" class="btn-link">{{ $t('home.learnMore') }} →</NuxtLink>
          </template>
        </UiCard>
      </div>
    </UiSection>
...
useTravelAgency()
useWebSite()
useBreadcrumbList([{ name: '首页', path: '/zh' }])
